...
documentation, and a real install (Cursor's Hooks panel showing ECC's registered hooks). Where Cursor
genuinely differs, it's called out — and where earlier claims were outdated (notably hooks), that's
corrected inline.
...
native events. A confirmed install shows ~27 hooks under an "ecc config" group, including
`preToolUse`, `preCompact`, and `sessionStart`. So the auto-format, compaction, and memory hooks
described in the Claude Code guide largely *do* fire on Cursor now. The remaining caveat is about
whether they're *effective*, not whether they *fire* — see Part 5.
...
```bash
ecc install --target cursor --profile minimal \
  --with lang:python \
...
cat ~/ecc-source/manifests/install-modules.json

# Preview the file plan before applying, so you see exactly what lands
ecc install --target cursor --profile minimal --with lang:python --with capability:machine-learning --dry-run
```
...
1. **`.cursor/AGENTS.md` pollution.** The installer may copy ECC's own `AGENTS.md` (which
   *describes ECC itself*) into your project's `.cursor/`. Because Cursor loads `AGENTS.md`
   ...
   the ECC plugin rather than your project. **After install, open `.cursor/AGENTS.md` and either
   delete it or replace its contents with a description of *your* project.** This is a known,
   reported bug — check whether your ECC version still does it.
...
the data root is shared (Part 3a). Copy the skill into both surfaces:

```bash
cp -r ~/ecc-source/skills/ck .cursor/skills/ck     # Cursor
cp -r ~/ecc-source/skills/ck ~/.claude/skills/ck   # Claude Code
```
...
a real **Hooks** settings panel (Settings → Hooks), and ECC registers its hooks there as native
events. A confirmed install shows ~27 hooks grouped under "ecc config," including multiple
`preToolUse` entries, `preCompact`, and `sessionStart`. So the quality and memory hooks from the
Claude Code guide largely fire on Cursor.

### What now works natively

- **`preToolUse`** — the pre-edit/pre-bash checks (including tmux/dev-server blocking and the
  format/lint reminders) fire as native Cursor hooks.
...
### The two real caveats (fire ≠ effective)

1. **`CLAUDE_PLUGIN_ROOT` must resolve.** The registered hook commands reference
   `process.env.CLAUDE_PLUGIN_ROOT` to find their scripts. If that variable isn't set in the
   environment Cursor launches from, the hooks will run but no-op (or fail quietly). Confirm it's
   set — this is the most likely reason a registered hook does nothing.
2. **Registered ≠ doing something useful.** Seeing 27 hooks in the panel proves they're wired, not
   that they're having an effect. The historical bug was hooks firing but capturing/injecting
   nothing. Verify by behavior: does `/instinct-status` fill up? Does a new session resume prior
   context? If yes, you're good.
...
# Discover valid component names, then selective-install (preview with --dry-run first)
npx ecc consult "mlops training model deployment" --target cursor
ecc install --target cursor --profile minimal --with lang:python --with capability:machine-learning
# Clean up: fix .cursor/AGENTS.md, check .cursor/agents/ for collisions
...
# export ECC_AGENT_DATA_HOME="$HOME/.ecc-shared"
# Confirm CLAUDE_PLUGIN_ROOT resolves so the registered hooks aren't no-ops:
...
| Hooks | 8 native event types, automatic | native Hooks panel (`preToolUse`, `preCompact`, `sessionStart`); ECC registers ~27 |
...
## Appendix — Reusable Cursor core, minimal

- **Install:** selective install per project — discover names with `npx ecc consult ... --target cursor`,
  then `ecc install --target cursor --profile minimal --with lang:python --with capability:machine-learning`
  (or the broad `./install.sh --target cursor python`). Never pass guessed `--modules` names.
...
- **Memory (shared across both tools):** keep Cursor and Claude Code on the **same** `ECC_AGENT_DATA_HOME`
  (don't set the Cursor-specific override). `ck` is path-keyed and instincts are git-remote-keyed, so
  the same repo resolves to the same memory in either tool. Drive `ck` manually (`/ck:resume` /
  `/ck:save`) and use the tools sequentially per project, not concurrently.
...
- **Hooks:** native on Cursor now — confirm `CLAUDE_PLUGIN_ROOT` resolves so they aren't no-ops;
  back up format-on-edit with Cursor format-on-save plus an agent-requested "verify your work" rule
- **ML/LLM (project-specific):** copy into `.cursor/skills/` only on projects that need them

**Honest caveats specific to Cursor:**
1. Hooks are native now (Settings → Hooks), but *registered ≠ effective*: confirm `CLAUDE_PLUGIN_ROOT`
   is set, and verify memory/learning by behavior (`/instinct-status`, session resume) rather than
   assuming. The historical bug was hooks firing yet capturing nothing.
2. Shared memory works because of how the stores are keyed, but the one collision surface is the flat
   session-summary file — use the two tools sequentially on a given project, not at the same time.
3. Check `.cursor/AGENTS.md` and `.cursor/agents/` after install for the two known collision/pollution
   bugs.
4. ECC is Claude-Code-first and ships weekly; the Cursor translation still lags in places. Verify
